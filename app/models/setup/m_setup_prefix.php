...
class M_setup_prefix extends CI_Controller{
	...
	function msave(){
		...
		if($get_arr['h_code'] != ""){
			...
			$modify_date = "";		
			$modify_by = "";			
		}///"sample"=>$get_arr['sample'],"sequence"=>$get_arr['sequence'], 	
	...
	function mshow(){
		$result = $this->db->query("SELECT
									...
									FROM
										z_setup_prefix AS prefix
									INNER JOIN set_park_package AS package ON prefix.package_typeno = package.package_typeno
									ORDER BY
										prefix.years ASC,
										prefix.prefix DESC")->result();
