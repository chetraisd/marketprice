...
class userprofile extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('setting/usermodelprofile','userprofile');
		$this->load->model('setting/usermodelprofile','park');
		$this->load->model('setting/usermodelprofile','gate');
	...
	public function index()
	{
		...
		$this->load->view('setting/user/addprofile',$data1);
	...
	function cEdite(){
		...
		$val_edit = $this->userprofile->mEdit($img_name);			
		$arr_show['tbl']= $val_edit;
		$arr_show['img']= $img_show;
		$arr_show['user_park']= $this->userprofile->getParkbyUser($img_name);
	...
	function cshow_tbl(){
		...
		$query = $this->userprofile->getuser($userid);
	...
	function getModule(){
		header('Content-type:text/x-json');
		echo json_encode($this->userprofile->getModule($this->input->post('moduleid')));
	}
	function getPage(){
		header('Content-type:text/x-json');
		echo json_encode($this->userprofile->getPage($this->input->post('moduleid'),$this->input->post('defpage')));
	}
}
