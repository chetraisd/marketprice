	...
	<div class="col-sm-12">
	    <div class="panel panel-default">
	      	<div class="panel-body">
				...
				<form  enctype="multipart/form-data" accept-charset="utf-8" method="post" id="defaultform" action='<?php echo site_url('setting/userprofile/saveuser');?>'>
						...
						<tr>
							...
							<td>
								<select name='cborole' id='cborole' class="form-control" >
									<?php
									echo $this->userprofile->getallrole();
									foreach ($this->userprofile->getallrole() as $role_row) {
...
<script type="text/javascript">
	$(function(){
		...
		$("#btnsubmit").click(function(){
			if($("#user_id").val()==""){
				alert("Don't have user to update !");
				return false;
			}
		});
		$("body").delegate("a#edite","click",function(){
			//var valEdite = $(this).attr("att_edit");
			var valEdite ="<?php echo $this->session->userdata('userid');?>";
			$("#cbopark option:selected").removeAttr("selected");
			$.ajax({
				type : "POST",
				url  : "<?php echo site_url('setting/userprofile/cEdite')?>",
				dataType: 'json',
				async: false,
				data: {
					val_edit : valEdite
				...
				success:function(data){
					...
					$("#moduleallow").val(data.tbl.def_dashboard);
					$("#defpage").attr("src",data.tbl.def_open_page);
		...
		$("body").delegate("#delete","click",function(){
			...
			if(conf == true){
				$.ajax({
					type : "POST",
					url  : "<?php echo site_url('setting/userprofile/cDeleteUser')?>",
					dataType: 'json',
					async: false,
					data: {
	...
	function show_inf(){
		$.ajax({
				type : "POST",
				url  : "<?php echo site_url('setting/userprofile/cshow_tbl')?>",
				dataType: 'json',
				async: false,
				data: {
					show_tbl : 1
	...
	function LoadModule(){
		    	$.ajax({
		    		type : "POST",
		    		url  : "<?php echo site_url('setting/userprofile/getModule')?>",
		    		dataType: 'json',
		    		async: false,
		    		data: {
		    			'roleid' : $("#cborole").val()
	...
	function LoadPage(){
		    	$.ajax({
		    		type : "POST",
		    		url  : "<?php echo site_url('setting/userprofile/getPage')?>",
		    		dataType: 'json',
		    		async: false,
		    		data: {
